...
//Insertion of a node at one end of linked list
...
Line 24 i.e. while(temp->next!=head){temp=temp->next;} establishes that after traversing element/variable has reached the first element.
...
/*[Ans.2]  
1)Computer Networking:Circular linked list is also used in computer networking for token scheduling.
